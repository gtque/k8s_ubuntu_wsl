...
while [ $# -gt 0 ]; do
  if [[ $1 == *"--"* ]]; then
    ...
    export helped=true
    echo "Usage:"
    echo -e "  --module [relative\\path\\\to\\module]\n\t  the path to the module in the modules directory of the example type to be deployed\n\t example: --module simpleApiServer"
    echo -e "  --example [relative\\path\\\to\\\example\\\type]\n\t  the path to the type of example being built, defaults to java.\n\t example: --example java"
    echo -e "  -clean\n\t  clean, defaults to false\n\t clean the build before the build"
    echo -e "  -docker\n\t  use dockerized build, defaults to false (cmd line gradlew build)\n\t dockerized builds may not be supported by all example types."
...
echo "building: ${example}/modules/${module}"
./${example}/build.sh "${_extra_args}"