...
3. Create a symbolic link to this project in the `/usr/local/bin` called wsl-local-k8s
   1. 
   ```
   cd /mnt/c/[path/to/cloned]/wsl-local-k8s
   sudo ln -s . /usr/local/bin/wsl-local-k8s
   ```
...
1. run `/usr/local/bin/wsl-local-k8s/tools/tool_config.sh`
...
4. run `/usr/local/bin/wsl-local-k8s/tools/docker_install.sh`
...
4. run `/usr/local/bin/wsl-local-k8s/microk8s/systemd_config.sh`
5. **exit and restart system (close the terminal window, open powershell and run wsl --shutdown)**
6. run `/usr/local/bin/wsl-local-k8s/microk8s/systemd_resolved.sh`
7. You are now ready to start setting up microk8s
## install microk8s
1. install microk8s by running: `/usr/local/bin/wsl-local-k8s/microk8s/microk8s_setup.sh`
...
7. Setup local kubeconfig: run `/usr/local/bin/wsl-local-k8s/microk8s/microk8s_kubeconfig.sh`
...
1. run `/usr/local/bin/wsl-local-k8s/tools/kubectl_and_helm.sh`
2. setup nfs run: `/usr/local/bin/wsl-local-k8s/ubuntu/nfs/setup.sh`
...
1. run `/usr/local/bin/wsl-local-k8s/infrastructure/setup.sh`
...
1. `/usr/local/bin/wsl-local-k8s/infrastructure/certs/selfsigned/setup.sh`
###### External
...
1. `/usr/local/bin/wsl-local-k8s/infrastructure/certs/external/install.sh`
1. edit `setup.sh` for your ca-certs
1. `/usr/local/bin/wsl-local-k8s/infrastructure/certs/external/setup.sh`
###### LetsEncrypt
...
1. edit `/usr/local/bin/wsl-local-k8s/infrastructure/certs/letsencrypt/k8s/00_cert_secret.yml`
  1. add your aws account access key
1. edit `/usr/local/bin/wsl-local-k8s/infrastructure/certs/letsencrypt/k8s/01_cert_issuer.yml`
  1. add your email
  1. set the access key
  1. set the zone id
  1. set the region
1. edit `/usr/local/bin/wsl-local-k8s/infrastructure/certs/letsencrypt/k8s/02_cert_manager.yml`
  1. set [your.domain] to a domain you control
1. `/usr/local/bin/wsl-local-k8s/infrastructure/certs/letsencrypt/setup.sh`
